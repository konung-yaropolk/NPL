...
class PCAPlot:
   ...
   #plotting methods
    ...
    def plot_covariance_matrix(self,
        ...
        sns.heatmap(self.covariance, annot=annot, cmap=cmap, xticklabels=self.feature_names, yticklabels=self.feature_names)
        plt.title(title)
        plt.show()
    def plot_principal_components_heatmap(self,
        ...
        if principal_components=='all':
            ...
            principal_components = list(map(lambda x: x-1, principal_components))
        if features=='all':
            features = range(self.n_features)
        else:
            features = list(map(lambda x: x-1, features))
            
        sns.heatmap(self.principal_components[principal_components].T[features], annot=True, cmap=cmap, xticklabels=list(map(lambda x: f'PC{x+1}', principal_components)), yticklabels=self.feature_names[features])
        plt.title(title)
        plt.show()
    
    def plot_explained_variance(self):
        plt.plot(range(1, len(self.explained_variance) + 1), self.explained_variance, marker='o', linestyle='--')
        plt.title('')
    ...
    def plot_transformed_data_2D(self,
        ...
        if len(loadings):
            ...
            for i in loadings:
                plt.arrow(0,0, load_vecs[i-1, x_axis-1], load_vecs[i-1, y_axis-1], head_width=0.03, head_length=0.06, linewidth=0.1, length_includes_head=True, color = 'r')
                plt.text(load_vecs[i-1,x_axis-1]* 1.15, load_vecs[i-1,y_axis-1] * 1.15, "Var"+str(i), color = 'g', ha = 'center', va = 'center')
           
        # labeling x and y axes
        PCX, PCY = self.transformed_data[:, x_axis-1], self.transformed_data[:, y_axis-1]
    ...
    def plot_transformed_data_3D(self,
        ...
        PCX, PCY, PCZ = self.transformed_data[:, x_axis-1], self.transformed_data[:, y_axis-1], self.transformed_data[:, z_axis-1]
        scalePCX = 1.0/(PCX.max() - PCX.min())
        scalePCY = 1.0/(PCY.max() - PCY.min())
        ...
        if len(loadings):
            ...
            for i in loadings:
                axis.quiver(
                    ...
                    load_vecs[i-1, x_axis-1], load_vecs[i-1, y_axis-1], load_vecs[i-1, z_axis-1], # <-- directions of vector
                    color = 'red', alpha = .8, lw = 2, arrow_length_ratio=0.1
                )
                axis.text(load_vecs[i-1, x_axis-1], load_vecs[i-1, y_axis-1], load_vecs[i-1, z_axis-1], "Var"+str(i), color='green')
